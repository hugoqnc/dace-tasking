...
def validate(A_row, A_col, A_val, x, H, W, repeat):
    for _ in range(repeat):
        ...
        if sp is not None:
            A_sparse = sp.csr_matrix((A_val, A_col, A_row), shape=(H, W))
            expected = A_sparse.dot(x)
            diff = np.linalg.norm(expected - b) / float(H)
            assert diff < 1e-6
    print("-"*50 + "Valid!")
...
if __name__ == "__main__":
    ...
    # why outer loop still multicore????
    i = 0
    ...
    validate(A_row, A_col, A_val, x, args.H, args.W, 10)
